...
export default function SchoolYearsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.year_name || !form.start_date || !form.end_date) { setError('Vui long dien day du thong tin'); return }
    setSubmitting(true); setError('')
    try {
      const res: ApiResult = editingId ? await updateSchoolYear(editingId, form) : await createSchoolYear(form)
      if (!res.success) { setError(res.error || 'Co loi xay ra'); setSubmitting(false); return }
      if (res.data) {
        setItems(prev => {
      ...
      setShowModal(false)
    } catch (err: any) { setError(err.message || 'Co loi xay ra') }
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <button
          onClick={openAdd}
          className="px-4 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors text-sm inline-flex items-center gap-2"
        >
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        {loading ? <div className="p-8 text-center text-gray-500">Dang tai...</div> : items.length === 0 ? <div className="p-8 text-center text-gray-500">Chua co nam hoc nao</div> : (
                  ...
                  <td className="px-3 md:px-6 py-3">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-2 py-1 text-xs rounded border border-blue-200 text-blue-600 hover:bg-blue-50 inline-flex items-center gap-1"
                      >
